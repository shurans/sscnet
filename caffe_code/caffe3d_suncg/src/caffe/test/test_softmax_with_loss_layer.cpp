...
class SoftmaxWithLossLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  SoftmaxWithLossLayerTest()
    ...
    std::vector<int> blob_shape(5);
    blob_shape[0] = 10;
    blob_shape[1] = 5;
    blob_shape[2] = 2;
    blob_shape[3] = 3;
    blob_shape[4] = 2;
    blob_bottom_data_ = new Blob<Dtype>(blob_shape);
    blob_shape[1] = 1;
    blob_bottom_label_ = new Blob<Dtype>(blob_shape);
    blob_bottom_weight_ = new Blob<Dtype>(blob_shape);
    // fill the values
  ...
  Blob<Dtype>* blob_bottom_weight_;
